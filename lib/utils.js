...
export function getFilePath(naivePath, seperator = PathLib.sep) {
  const jsonPath = naivePath.split(seperator).filter((path) => path !== '')
...
export function handleBigFile(txsContent, filePath, content) {
  let delimitersLength = 7 + filePath.length

  while (content.length > 0) {
    // Split the file
    const part = content.slice(0, MAX_FILE_SIZE - delimitersLength)
    content = content.replace(part, '')
    // Set the value in transaction content
    const txContent = {
      content: {},
      size: part.length + delimitersLength,
      refPath: []
    }
...
//   while (content.length > MAX_FILE_SIZE - delimitersLength) {
//     // Split the file
//     const part = content.slice(0, MAX_FILE_SIZE - delimitersLength)
//     content = content.replace(part, '')
//     // Set the value in transaction content
...
//   }
//   if (content.length > 0) {
//     handleNormalFile(txsContent, filePath, content)
//   }
// }
...
export function getRefTxContent(txsContent, transactions, metaData, sslCertificate) {
  ...
  transactions.forEach((tx) => {
    if (!tx.address) throw new Error('Transaction is not built')

    const txContent = txsContent.find(val => isEqual(val.content, tx.data.content))

    if (!txContent) throw new Error('Transaction content not expected')
    // For each filePath
