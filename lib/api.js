...
export default class AEWeb {
  ...
  addFile(naivePath, data) {
    ...
    const hash = nativeCryptoLib.createHash('sha-1').update(data).digest('hex')
    let content = zlib.gzipSync(data).toString('base64url')

    const jsonPath = naivePath.split(path.sep)
    if (jsonPath[0] === '') jsonPath.shift()
    const filePath = `${jsonPath.join('/')}`

    this.metaData[filePath] = {hash: hash,size: size,encoding: '',addresses: []}
...
function getMetaData(txsContent, transactions, metaData, sslCertificate) {
  ...
  transactions.forEach((tx) => {
    ...
    return txContent.refPath.forEach((filePath) => {
      var { addresses } = metaData[filePath]
      addresses.push(address)
      metaData[filePath]['addresses'] = addresses
      metaData[filePath]['encoding'] = 'gzip'
    })
  })
