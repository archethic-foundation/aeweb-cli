...
module.exports = {
    ...
    hmac2: function(Files,algo,myseed,Seed,Address)
    {
        for (let i = 0; i < Files.length; i++) {
            const hmac = crypto.createHmac(algo, myseed);
            hmac.update(Files[i])
    ...
    folder_waitConfirmations: function (transaction,address,endpoint,x) {
        archethic.waitConfirmations(transaction.address, endpoint, function (nbConfirmations) {
            if (nbConfirmations == 1) {
                console.log(chalk.gray(x + " deployed successfully"))
                console.log(chalk.blue(endpoint + "/api/last_transaction/" + address + "/content?mime=" + mime.getType(x)))
            }
            console.log(chalk.magenta("Transaction confirmed with " + nbConfirmations + " replications"))
        })
    },

    website_waitConfirmations: function (transaction,address,endpoint,x) {
        archethic.waitConfirmations(transaction.address, endpoint, function (nbConfirmations) {
            if (nbConfirmations == 1) {
